...
class EventResultService(
    ...
    private fun connect() {
        Database.connect(
            url = "jdbc:mysql://localhost:3306/pcea?useSSL=false&serverTimezone=Asia/Tokyo&rewriteBatchedStatements=true&allowPublicKeyRetrieval=true",
    ...
    fun getEventResults(from: LocalDate, to: LocalDate): List<HoldingEventResult> {
        connect()
        return transaction {
            val holdingEventIdsQuery = HoldingEventRecordTable
    ...
    fun searchEventResults(from: LocalDate, to: LocalDate, page: Int, limit: Int): SearchPaginatedEventResultResponse {
        return transaction {
            ...
            val summaries = holdingEvents.sortedBy { it.eventDate }.reversed().map { holdingEvent ->
                ...
                EventResultSummary(
                    ...
                    results = holdingEventResults.sortedBy { it.rank }.map {
